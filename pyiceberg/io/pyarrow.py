...
from pyiceberg.cache import Cache, NoCache
...
def project_table(
    ...
    Args:
        ...
        cache (Cache): the cache used (default is no cache).
    ...
    for i, task in enumerate(tasks):
        ...
        if cache.get(path_) is not None:
            ...
            total_row_count += len(result)
        else:
            tables.append(None) # no result yet
            ...
            # future's result will end up at i-th table and cached as path_
            futures_index[future] = (i, path_)
